...
const SelectImageModal: React.FC<SelectImageModalProps> = ({ isOpen, onClose, onSelect, dataService }) => {
  ...
  useEffect(() => {
    if (selectedKey) {
      const url = Utils.cleanURL(editor_config.MediaProxy,selectedKey);
  ...
  return (
    <Dialog open={isOpen} onClose={onClose} maxWidth="sm" fullWidth>
          ...
          {imageKeys.map(key => (
            <ListItemButton selected={selectedKey === key} onClick={() => setSelectedKey(key)} key={key}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <img
                  src={Utils.cleanURL(editor_config.MediaProxy,key)}
                  alt={key}
      ...
      <DialogActions>
        ...
        <Button onClick={() => selectedKey && onSelect(`${Utils.cleanURL(editor_config.MediaProxy, selectedKey)}`, selectedKey, width, height)} disabled={!selectedKey} variant="contained">Select</Button>
      </DialogActions>
    </Dialog>
  );
};
